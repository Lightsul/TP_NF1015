...
#pragma endregion
	...
	auto itAya = itAlucard;
	while (itAya != listeHeros.end() && (*itAya).getNom() != "Aya Brea")
	{
		itAya.avancer();
	}

	// TODO: Ajouter un hero bidon à la liste avant Aya Brea en vous servant de l'itérateur.
	Heros bidon("Bidon", "jeuBidon", "ennemiBidon");
	listeHeros.insert(itAya, bidon);
	auto itBidon = trouverParNom(listeHeros, "Bidon");
	if (itBidon != listeHeros.end())
	{
		cout << separateurSections << "Hero bidon:" << endl;
		cout << separateurElements;
		(*itBidon).changerCouleur(cout, 0);
		(*itBidon).afficher(cout);
	}
	// TODO: Assurez-vous que la taille de la liste est correcte après l'ajout.
	cout << separateurElements << endl;
	cout << "Taille de la liste après insertion : " << listeHeros.size() << endl;
	...
	auto itMario = itAya;
	while (itMario != listeHeros.begin() && (*itMario).getNom() != "Mario")
	{
		itMario.reculer();
	}

	auto itApresMario = listeHeros.erase(itMario);
	cout << "Après Mario : " << (*itApresMario).getNom() << endl;
	// TODO: Assurez-vous que la taille de la liste est correcte après le retrait.
	cout << "Taille de la liste après effacement : " << listeHeros.size() << endl;
	...
	cout << separateurSections << "Liste des héros (avec itérateur) :" << endl;

	for (auto it = listeHeros.begin(); it != listeHeros.end(); it.avancer())
	{
		cout << separateurElements;
		(*it).changerCouleur(cout, 0);
		(*it).afficher(cout);
	}

	// TODO: Refaite le même affichage mais en utilisant une simple boucle "for" sur intervalle.
	cout << separateurSections << "Liste des héros (boucle for sur intervalle) :" << endl;

	for (auto &hero : listeHeros)
	{
		cout << separateurElements;
		hero.changerCouleur(cout, 0);
		hero.afficher(cout);
	}

	// TODO: Utilisez un conteneur pour avoir les héros en ordre alphabétique (voir point 2 de l'énoncé).
	auto cmpNom = [](const Heros &a, const Heros &b)
	{
		return a.getNom() < b.getNom();
	};

	std::set<Heros, decltype(cmpNom)> herosTries(cmpNom);

	for (auto &hero : listeHeros)
	{
		herosTries.insert(hero);
	}

	cout << separateurSections << "Héros triés alphabétiquement :" << endl;

	for (auto &h : herosTries)
	{
		cout << separateurElements;
		h.changerCouleur(cout, 0);
		h.afficher(cout);
	...
	// 2.2 — Trouver et afficher Aya Brea dans le conteneur trié
	Heros cleRecherche("Aya Brea", "", ""); // seul le nom est utilisé pour la comparaison

	auto itTrouve = herosTries.find(cleRecherche);
	if (itTrouve != herosTries.end())
	{
		cout << separateurElements;
		(*itTrouve).changerCouleur(cout, 0);
		(*itTrouve).afficher(cout);
	}

	cout << "Aya Brea Trouvée " << endl;
	/* Complexité moyenne : O(log n)
	Explication : std::set est implémenté comme un arbre binaire de recherche équilibré.
	La recherche se fait en suivant un chemin de hauteur log(n), donc en temps logarithmique.
	*/

	/* 2.3 : Le conteneur utilisé en 2.1 (std::set) permet une recherche plus rapide.
	En effet, std::set est un arbre binaire de recherche équilibré,
	ce qui donne une recherche en O(log n).
	La liste chaînée doit être parcourue séquentiellement, donc O(n).
	Pour une recherche par nom, std::set est donc beaucoup plus performant.
	*/
